...
signal faltando_um_hit_para_morrer(capanga: Node)
...
@export var attack_range_x: float = 42.0
@export var attack_range_y: float = 55.0

@export var zona_morta_player_x: float = 8.0

@export var distancia_minima_entre_capangas: float = 38.0
@export var margem_y_capanga_na_frente: float = 42.0

@export var hurt_knockback_x: float = 18.0
@export var hurt_knockback_y: float = 0.0
@export var hurt_knockback_freio: float = 260.0

# Todo golpe mostra reação visual.
# Mas o stun longo só acontece a cada alguns hits.
@export var hits_para_stun: int = 2
@export var hit_reaction_duration: float = 0.22
@export var hit_flash_duration: float = 0.08
@export var hit_micro_knockback_x: float = 10.0

# Contra-ataque controlado, sem spam.
@export var hits_para_contra_ataque: int = 3
@export var contra_ataque_delay: float = 0.55
@export var contra_ataque_cooldown: float = 1.4
...
@onready var icone_capanga: Node2D = $IconeCapanga
...
var can_attack := true
var aviso_um_hit_emitido := false
...
func _ready() -> void:
	add_to_group("capanga_agressivo")
...
func activate(target: Node2D) -> void:
	...
	if target == null or not is_instance_valid(target):
		push_warning(name + " recebeu player inválido em activate().")
		return
...
func adicionar_excecoes_de_colisao() -> void:
	...
	for outro_capanga in get_tree().get_nodes_in_group("capanga_agressivo"):
		if outro_capanga == self:
			continue

		if outro_capanga is PhysicsBody2D:
			add_collision_exception_with(outro_capanga)
			outro_capanga.add_collision_exception_with(self)

	var prefeito := get_tree().get_first_node_in_group("prefeito")
	if prefeito is PhysicsBody2D:
		add_collision_exception_with(prefeito)
		prefeito.add_collision_exception_with(self)

	var filha_liz := get_tree().get_first_node_in_group("filha_liz")
	if filha_liz is PhysicsBody2D:
		add_collision_exception_with(filha_liz)
		filha_liz.add_collision_exception_with(self)
...
func tem_capanga_na_frente() -> bool:
	if player == null or not is_instance_valid(player):
		return false

	var direcao_para_player := signf(player.global_position.x - global_position.x)

	if direcao_para_player == 0.0:
		return false
	...
	for outro_capanga in get_tree().get_nodes_in_group("capanga_agressivo"):
		...
		if not is_instance_valid(outro_capanga):
			continue

		var morto = outro_capanga.get("dead")
		if morto != null and morto == true:
			continue

		var ativo = outro_capanga.get("active")
		if ativo != null and ativo == false:
			continue

		if not outro_capanga is Node2D:
			continue

		var outro := outro_capanga as Node2D
		var diferenca_para_outro := outro.global_position - global_position
		var diferenca_player_para_outro := player.global_position.x - outro.global_position.x

		if absf(diferenca_para_outro.y) > margem_y_capanga_na_frente:
			continue

		if signf(diferenca_para_outro.x) != direcao_para_player:
			continue

		if signf(diferenca_player_para_outro) != direcao_para_player:
			continue

		if absf(diferenca_para_outro.x) <= distancia_minima_entre_capangas:
			return true
...
func take_hit(damage: int = 1) -> void:
	...
	if not aviso_um_hit_emitido and hits_taken >= max_hits - 1:
		aviso_um_hit_emitido = true
		faltando_um_hit_para_morrer.emit(self)
