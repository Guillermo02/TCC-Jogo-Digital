...
@export var pausa_luz_apagada: float = 2.0
...
func configurar_luz() -> void:
	if luz_area == null:
		push_warning("Droninho: LuzArea não encontrada. Crie um Area2D chamado LuzArea.")
		return

	if luz_visual == null:
		push_warning("Droninho: LuzVisual não encontrada. Coloque o Polygon2D dentro de LuzArea e renomeie para LuzVisual.")
		return

	if colisao_luz == null:
		push_warning("Droninho: CollisionPolygon2D não encontrado dentro de LuzArea.")
		return
